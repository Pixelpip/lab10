...
function DataFetcher() {
  ...
  return (
    <div>
      ...
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search..." /> {load ? <div>Loading...</div> : (
        <table>
          <thead><tr><th>Name</th><th>Email</th><th>City</th></tr></thead> <tbody>
            {filtered.length ? filtered.map(({ id, name, email, address }) => (
              <tr key={id}><td>{name}</td><td>{email}</td><td>{address.city}</td></tr>)) : <tr><td colSpan="3">No results</td></tr>}
          </tbody>
        </table>
      )}
